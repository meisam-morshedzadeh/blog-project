...
&& isset($_POST['password']) && $_POST['password']!=='')
{
    ...
    if($_POST['password']===$_POST['confirm'])
    {
        ...
        {
            $query ="SELECT * FROM blog_project.users WHERE email=?";
            ...
            if($user === false)
            {
                ...
                $query ="INSERT INTO blog_project.users SET email=?, first_name=?, last_name=?, password=?, created_at=NOW()";
                $statement = $pdo->prepare($query);
    ...
    {
        $error = 'رمز عبور و تاییدیه آن مطابقت ندارد';
    }
}
else
{
    if(!empty($_POST))
    {
        $error = 'پر کردن همه فیلدها اجباری است';
    }
}
