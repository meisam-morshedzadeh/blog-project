...
{
    $query ="SELECT * FROM blog_project.categories WHERE id=?";
    $statement = $pdo->prepare($query);
    ...
    if(isset($_FILES['image']) && $_FILES['image']['name']!=='')
    {
        ...
        {
            $query ="UPDATE blog_project.posts SET title=?, cat_id=?, body=?, image=?, updated_at=NOW() WHERE id=?;";
            $statement = $pdo->prepare($query);
    ...
    else 
    {
        if($category !== false)
        {
            $query ="UPDATE blog_project.posts SET title=?, cat_id=?, body=?, updated_at=NOW() WHERE id=?;";
            $statement = $pdo->prepare($query);
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <section class="form-group">
                            ...
                            <select class="form-control" name="cat_id" id="cat_id">
                                <?php
                                    global $pdo;
                                    $query ="SELECT * FROM blog_project.categories";
                                    $statement = $pdo->prepare($query);
                                    $statement->execute();
                                    ...
                                    {
                                ?>
                                <option value="<?= $category->id ?>" <?php if($post->cat_id==$category->id) echo 'selected' ?>><?= $category->name ?></option>
                                <?php 
                                    }
                                ?>
                        </select>
